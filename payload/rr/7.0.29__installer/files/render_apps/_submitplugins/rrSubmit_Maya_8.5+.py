...
class rrDelightRenderPass:
    def __init__(self):
        ...
        self.channelExts = []
        self.RequiredLicenses = "_3delight;"
...
class rrMayaLayer:
    ...
    def getLayerSettings(self,Layer,DatabaseDir,SceneName,MayaVersion,isLayerRendering):
        ...
        if (self.renderer!="mayaSoftware"):
            self.RequiredLicenses=self.renderer+";"
        nbRenderableCams=0;
        for c in range(0, len(self.tempCamNames)):
    ...
    def getImageOut2009(self):
        ...
        if ((self.renderer=="arnold") and (cmds.getAttr('defaultArnoldRenderOptions.aovMode')!=1) and (self.imageFileName.find("<Channel>")<0)):
...
class rrSceneInfo:
    ...
    def getSceneInfo(self):
        ...
        version=str(version/100)+"."+str(version % 100/10)
...
def rrWritePassToFile(fileID, Layer, DPass, sceneInfo,camera,LocalTextureFile):
        ...
        rrWriteNodeStr(fileID,"RequiredLicenses", DPass.RequiredLicenses)
...
def rrWriteLayerToFile(fileID,Layer, channel,sceneInfo,camera,LocalTextureFile):
        ...
        rrWriteNodeStr(fileID,"RequiredLicenses", Layer.RequiredLicenses)
        rrWriteNodeStr(fileID,"Camera",camera)
...
class rrPlugin(OpenMayaMPx.MPxCommand):
    ...
    def writeTextureList(self):
        ...
        for tex in texList:
            ...
            rrWriteNodeStr(fileID,"Original", texFileName )
            fileID.write("</File>\n")
    ...
    def doIt(self, arglist):
        print ("rrSubmit v 7.0.01+6.02.41")
