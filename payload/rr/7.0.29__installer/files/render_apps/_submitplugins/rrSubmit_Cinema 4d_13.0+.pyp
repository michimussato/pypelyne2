...
class RRDialog(c4d.gui.GeDialog):
    ...
    def CreateLayout(self):
        self.SetTitle("Tiled RRSubmit v 7.0.11")
...
class RRSubmit(c4d.plugins.CommandData):
    ...
    def getLanguage(self):
        ...
        CinemaPath=CinemaPath+PD() + "resource"+PD()+"modules"+PD()+"newman"+PD()+"res"+PD()+"strings_"+str(language).lower()+PD()+"c4d_strings.str"
        ...
        strfile.close()   
        CinemaPath= os.path.dirname(os.path.dirname(__file__))
        CinemaPath=CinemaPath+PD() + "resource"+PD()+"strings_"+str(language).lower()+PD()+"c4d_strings.str"
        strfile = open(CinemaPath)
        for sline in strfile :
            sline= sline.rstrip()
            svalue=""
