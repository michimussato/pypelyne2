	...
	,fn getCameras =
		(
			...
			for c in 1 to camobjs.count do 	(
				if ((classof camobjs[c] == TargetCamera) or   (classof camobjs[c] == XRefObject) or  (classof camobjs[c] == FreeCamera)  or  (classof camobjs[c] == Physical) or  (classof camobjs[c] == VRayPhysicalCamera) or  (classof camobjs[c] == VRayDomeCamera)) then
					(
...
(
	...
	,fn getCameras =
		(
			...
			rrSI.Cameras =#()
			for c in 1 to camobjs.count do 	(
			print ("cam type found: "+ ((classof camobjs[c]) as string))
				if ((classof camobjs[c] == TargetCamera) or   (classof camobjs[c] == XRefObject) or  (classof camobjs[c] == FreeCamera)  or  (classof camobjs[c] == Physical) or  (classof camobjs[c] == VRayPhysicalCamera) or  (classof camobjs[c] == VRayDomeCamera)) then
					(
					posi = findString camobjs[c].name "Target"
					if (posi == undefined) then append rrSI.Cameras camobjs[c].name
