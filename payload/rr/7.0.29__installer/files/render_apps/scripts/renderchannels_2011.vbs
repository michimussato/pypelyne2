...
Dim fso, f, filename, PD ,dirname, slen, spos , tempstrg,oldpad,list,OldMdlLvl,chfname,chdname,fbuffer, renderer, fileSystem, ImagesList, Image, imgFileName, fnamechar1, fnamechar2, oPass, orgFileName, iloop
...
'''commented out: Set fileSystem = CreateObject( "Scripting.FileSystemObject" )
'''commented out: Set ImagesList=EnumElements("Sources.Image")
'''commented out: IF TypeName(ImagesList) <> "Nothing" THEN
'''commented out:     FOR EACH Image IN ImagesList
'''commented out: 		imgFileName = GetValue ( Image & ".FileName")
'''commented out: 		spos= InStr(1,imgFileName ,"noIcon.pic",vbTextCompare)
'''commented out: 		IF (spos=0) THEN
'''commented out: 			imgFileName=XSIUtils.ResolveTokenString(imgFileName,0,false)
'''commented out: 			imgFileName= Replace(imgFileName, "/", "\")
'''commented out: 			fnamechar1 = left(imgFileName,1)
'''commented out: 			fnamechar2 = right(left(imgFileName,2),1)
'''commented out: 	
'''commented out: 			IF (fnamechar2 <> ":") AND (fnamechar2 <> "\") AND (fnamechar1 <> "\") THEN
'''commented out: 				imgFileName= Application.ActiveProject2.Path & "\" & imgFileName
'''commented out: 			END IF
'''commented out: 			IF ((InStr(1,imgFileName ,"[",vbTextCompare)=0) AND NOT fileSystem.FileExists( imgFileName )) THEN
'''commented out: 				logmessage "Info: Image file does not exist: " & imgFileName
...
if not (IsNull(fname) or (len(fname)=0)) then
  ...
  if (not (IsNull(pad) or (len(pad)=0)) and (pad>4)) then
    For iloop= 1 to pad 
      fname= fname & "#"
    next
  else
